...
export default function WBSGanttPage() {
  ...
  const runCPM = () => {
    ...
    try {
      ...
      // Update task critical status
      const criticalTaskIds = new Set(result.criticalTasks.map(t => t.id));
      
      // Update data with critical path information
      if (data.wbsData?.items) {
        const updateCriticalStatus = (items: any[]): any[] => {
          return items.map(item => ({
            ...item,
            isCritical: criticalTaskIds.has(item.id),
            children: item.children ? updateCriticalStatus(item.children) : undefined
          }));
        };

        const updatedWbsData = {
          ...data.wbsData,
          items: updateCriticalStatus(data.wbsData.items)
        };

        updateData({ wbsData: updatedWbsData });
      }
  ...
  const renderTaskBar = (task: any) => {
    ...
    return (
      <div
        className={`task-bar ${isCritical ? 'critical' : ''} ${isSummary ? 'summary' : ''}`}
        style={{
          position: 'absolute',
          left: `${left}px`,
          width: `${width}px`,
            ...
            <table className="data-table">
              ...
              <tbody>
                {flatTasks.map((task, index) => {
                  ...
                  return (
                    <tr key={task.id} className={`${rowClass} border-b border-slate-800 transition-colors`}>
