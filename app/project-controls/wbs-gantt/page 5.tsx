            ...
            <table className="data-table">
              ...
              <tbody>
                {flatTasks.map((task) => {
                  ...
                  const projected = task.projectedHours ? Math.round(task.projectedHours) : '-';
                  const actual = task.actualHours ? Math.round(task.actualHours) : '-';
                  const baselineCost = task.baselineCost ? `$${Math.round(task.baselineCost).toLocaleString()}` : '-';
                  const actualCost = task.actualCost ? `$${Math.round(task.actualCost).toLocaleString()}` : '-';
                  ...
                  return (
                    <tr key={task.id} className="border-b border-slate-800 transition-colors">
                      ...
                      <td className="px-3 py-2 text-right text-xs font-mono">{projected}</td>
                      <td className="px-3 py-2 text-right text-xs font-mono">{actual}</td>
                      <td className="px-3 py-2 text-right text-xs font-mono">{baselineCost}</td>
                      <td className="px-3 py-2 text-right text-xs font-mono">{actualCost}</td>
          ...
          <div ref={containerRef} style={{ overflowX: 'auto', overflowY: 'auto', maxHeight: '600px' }}>
            <table style={{ minWidth: '100%' }}>
              <thead style={{ position: 'sticky', top: 0, backgroundColor: 'var(--bg-primary)', zIndex: 10 }}>
                <tr>
                  <th style={{ width: '400px', padding: '8px', textAlign: 'left', borderBottom: '1px solid var(--border-color)' }}>Task</th>
