...
interface TransformWBSItem {
  ...
  wbsCode: string;
  name: string;
  type: 'portfolio' | 'customer' | 'site' | 'project' | 'phase' | 'unit' | 'task' | 'sub_task' | 'milestone';
  itemType: string;
  startDate?: string;
  endDate?: string;
  progress?: number;
...
export function buildWBSData(data: Partial<SampleData>): { items: any[] } {
  ...
  return memoize('buildWBSData', () => {
    ...
    const buildProjectNode = (project: any, projectWbs: string): TransformWBSItem => {
      ...
      // 1. Check for Unified Task Hierarchy (V13+)
      ...
      if (isUnifiedHierarchy) {
        ...
        return projectItem; // Early return for V13 hierarchy
      }
    ...
    // Build hierarchy - Projects are now the top-level items
    projects.forEach((project: any, pIdx: number) => {
      // Use index + 1 as initial WBS code (will be re-indexed later anyway)
      items.push(buildProjectNode(project, String(pIdx + 1)));
    });
