...
export function convertProjectPlanJSON(data: Record<string, unknown>, projectIdOverride?: string): Partial<SampleData> {
  ...
  // Detect V13 Unified Task List Format (from new mpp_parser.py)
  // The V13 parser returns a flat list of tasks in 'tasks' key, with 'outline_level' and 'is_summary'
  if (Array.isArray(data.tasks) && data.tasks.length > 0 && ('outline_level' in (data.tasks[0] as any) || 'wbs' in (data.tasks[0] as any))) {
    const tasks = data.tasks.map((t: any, i: number) => {
      const taskId = t.id || t.unique_id || `TSK-${(i + 1).toString().padStart(4, '0')}`;
      return {
        id: taskId,
        taskId: taskId,
        projectId: projectIdOverride || t.project_id || '',
        ...
        assignedResourceType: (t.assigned_resource_type as 'specific' | 'generic') || 'generic',
        ...
        earlyStart: t.early_start || null,
        earlyFinish: t.early_finish || null,
        lateStart: t.late_start || null,
        lateFinish: t.late_finish || null,
        ...
        projectedHours: t.baseline_work || 0,

        baselineHours: t.baseline_work || 0,
        actualHours: t.actual_work || 0,
        baselineCost: t.baseline_cost || 0,
        actualCost: t.actual_cost || 0,

        comments: t.notes || '',
        predecessorId: t.predecessors?.length ? t.predecessors[0].id : null,
        predecessorRelationship: t.predecessors?.length ? (t.predecessors[0].type || 'FS') : null,
    ...
    // If we need a project container
    let projects: any[] = [];
    if (!projectIdOverride && data.project) {
      const p: any = data.project;
      projects = [{
        projectId: p.id || `PRJ-${now}`,
        name: p.name || 'Imported MPP Project',
      ...
      // We assume project exists, but we can return it to ensure update
      projects = [{
  ...
  // Handle tasks (Legacy V12 or standard)
